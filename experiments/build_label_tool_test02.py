# -*- coding: utf-8 -*-
"""Build the labelling tool for TEST 2's confirmed detections.

Test 2 switched off the three anti-reflection rules and recall went from 37% to
50%, with the confirmed set growing from 3.8 marks a photo to 7.5. Precision
came out at 96.1% -- but 40 of those confirmed detections had never been judged
by anyone, because the hand labelling only ever covered what the rules ALLOW
through. A precision figure resting on three quarters of its own evidence is not
one to decide on.

So this asks about the detections the relaxed rules let in, and nothing else:

  - the three rules off, thresholds unchanged (test 2's config 5)
  - only marks CONFIRMED in both shots, since those are the ones that count
  - not the ones on a pen mark: correct by construction, never worth your time
  - every verdict you have already given, from both earlier rounds, carried
    over by position

The angle is corrected before matching and the window is the tightened 2
degrees, matching what now runs on the server.

Usage:  python build_label_tool_test02.py [how many records]
"""
...
from test_01_loosen_then_confirm import WINDOW, analysed, gt_for, pick_sides
from test_02_drop_glare_rules import CONFIGS
from tune_alignment import offsets, refine

HERE = os.path.dirname(os.path.abspath(__file__))
TEMPLATE = os.path.join(HERE, "label_page.html")
OUT = os.path.join(HERE, "label_tool_test02")
SET = "test02"
CONFIG = "5_none_of_the_three"

# every round of verdicts given so far, so nothing is asked twice
